...
$$\mathbb{R}^2$$ as opposed to depending only on intrinsic properties of $$S^1$$. 
...
* It is also the *unique* continuous distribution with the maximum entropy for a 
...
* It solves the diffusion equation. Given $$f(x | \mu, t) = 
\frac{1}{\sqrt{2\pi t}} e^{-(x-\mu)^2 / 2 t}$$ density of the Normal distribution
written as a function of $$t = \sigma^2$$.

We have $$\partial_t f(x | \mu, t) = \frac{1}{2} \partial_x^2 f(x | \mu, t)$$, so
the density solves the diffusion equation - and as $$t \rightarrow 0$$ the density
...
distribution of their sum $$X+Y$$ is in the same family? Consider 2 distributions
with on the circle with density $$\rho_X(x), \rho_Y(y)$$ and their respective 
fourier transforms $$\rho_X(n), \rho_Y(m)$$ for $$m \in \mathbb{Z}$$. The condition
...
We further must require that 

$$\rho(x) = \sum_n \exp{i 2\pi n x} \rho_X(n)$$ is positive for all $$x$$.

Or equivalently that $$\rho_X(n)$$ is the fourier transform of a non-negative function.
...
vanishes - and there is even a wider range of possible values for $$\beta$$.
...
Diffusion Equation -> Theta Function
======================================
...
$$\partial_t \Theta(x,it) = -n^2 \Theta(x,it) = \partial_x^2 \Theta(x, it)$$
...
part of a wider family of distribution obtained by 'wrapping' a distribution on the real
line around circle.

I am biased towards this it presents a unique candidate for the 'Normal' disribution
on the circle - and more generally in any situation where we can define the diffusion 
equation (Riemannain manifold for instance).

Statistical Inference with Theta Distribution.
===============================================

Given a probability distribution from this family $$\theta(x - \mu, t)$$ (with 
the maxima of the density specified by $$\mu$$ and the width specified by t) 
it is natural (within statistics at least) to ask how we estimate parameters 
given a discrete set of samples. The problem of estimating the maxima of the
 distribution alone is quite interesting. Suppose we have a set of $N$ samples 
 $x_n$ drawn from a distribution with density $$\theta(x - \mu, t)$$
with known $$t$$ and unkown $$\mu$$. We want to obtain a best estimate of 
$$\mu$$. Followign an analogous path to the Normal distribution where the max 
likelihood estimate is the mean of the observations we seek a ML estimate of $
$\mu$$. This provides a nice concrete example of how the notion of a 'mean value' 
extends beyond $$\mathbb{R}$$. 
...
on the circle we have $$\mu = \frac{1}{N} x_n + \frac{2\pi r}{N} for $$r \in
[0,N-1]$$. There are $$N-1$$ possible 'mean values'. (This is quite generic
...
of theta functions, much rearranging and mangling of summations, or an elegant argument I 
haven't found yet... we find that the product of 2 theta densities with same
width $t$ is

$$\theta(t, x_1 - \mu) \theta(t, x_2 - \mu) = \frac{1}{2} \sum_{r=0}^{1} 
\theta(t/2, \frac{x_1-x_2}{2} + r\pi) \theta(t/2, (x_1+x_2)/2 + r\pi - \mu)$$
...
term is peaked at the possible mean values.
...
$$\frac{1}{N} \sum_{r=0}^{N-1} \theta(/frac{t}{N(N-1)}, \frac{x' - x_N + 2\pi r}{N}) 
\theta(t/N, \frac{(N-1)x' + x_N + 2\pi r}{N} - \mu)$$
...
\theta(\frac{t}{b(a+b)}, \frac{x - x' + 2\pi r}{a+b}) 
\theta(\frac{t+t'}{tt'}, \frac{a x + b x' + 2\pi a r}{a+b})$$
...
defining $$q = n+m$$ and $$p = bn - am$$. With this choice the exponential becomes
...
for $$s = \frac{t}{b(a+b)}$$, $$s' = \frac{at}{a+b}$$
...
$$\phi^{-1}(p,q) = (\frac{aq+p}{a+b}, \frac{bq-p}{a+b})$$

so the image of a generic pair of integers $$p,q$$ under the inverse mapping will be rational.
A pair $$(p,q)$$ is in the image of $$\phi$$ iff $$aq+p \rm{mod} (a+b) == 0$$ and $$bq-p \rm{mod} (a+b)==0$$.
