...
\def\p{\phi}
\def\vp{\varphi\def\H{{\cal H}}
\def\Hkg{\H_{\rm kin}^{\rm grav}}
\def\Hk{\H_{\rm kin}}
\def\Hp{\H_{\rm phy}}
\def\Hkm{\H_{\rm kin}^{\rm matt}}
\def\h{\hat }
\def\b{\bar}
\def\ub{\underbar}
\def\ul{\underline}
...
$$
\begin{equation}
A_a^i =: c^i (L^i)^{-1} \fw_a^i \quad E^a_i =: p_i L_i V_o^{-1} \sqrt{{}^o q} {}^o e^a_i
\end{equation}
$$
...
$$
\begin{equation}
p_1 = \sgn(a_1) |a_2 a_3| L_2 L_3   \quad p_2 = \sgn(a_2) |a_1 a_3| L_1 L_3  \quad p_3 = \sgn(a_3) |a_1 a_2| L_1 L_2
\end{equation}
$$
...
$$
\begin{equation}
 \label{hol} h_1^{(\ell)} (c_1,c_2,c_3) = \cos \f{c_1\ell}{2}
\mathbb{I} + 2 \sin \f{c_1 \ell}{2} \tau_1
\end{equation}
$$
...
$$
\begin{equation}
|| \Psi ||^2 = \underset{\alpha \rightarrow \infty}{lim} \frac{1}{2\alpha} \int_{-\alpha}^{\alpha} \dd c |\Psi(c)|^2
\end{equation}
$$
...
$$
\begin{equation}
\tilde{\Psi}(p) =  \underset{\alpha \rightarrow \infty}{lim} \frac{1}{2\alpha} \int_{-\alpha}^{\alpha} \dd c \,e^{i c p} \Psi(c)
\end{equation}
$$
...
$$
\begin{equation} \mathcal{H} = \f{E^a_iE^b_j}{16\pi
...
\mathrm{Tr}\left(\f{h_{\Box_{ij}}- \mathbb{I}} {Ar_\Box}
...
$$
\begin{equation} \label{mubar} \bar\mu_1 = \sqrt\f{|p_1|\Delta\,\lp^2}{|p_2p_3|},
\quad \bar\mu_2 = \sqrt\f{|p_2| \Delta\,\lp^2}{|p_1p_3|}, \quad
\bar\mu_3 = \sqrt\f{|p_3|\Delta\,\lp^2}{|p_1p_2|}\, . \end{equation}
$$
...
Much like in the FLRW models the action of these holonomies is simplified if we change variables to $\l_i$ defined by,

$$
\begin{equation} \l_i\, =\, \f{\sgn(p_i)\,\sqrt{|p_i|}}{(4\pi |\gamma|\,
\sqrt\Delta\, \lp^3)^{1/3}}\, , \end{equation}
$$
...
under these change of variables our Hilbert space is now functions of $\l_1,\l_2$, and $v$, all restricted to be positive, with finite norm.

$$
\begin{equation}
|| \Psi ||^2 = \sum_{\l_1, \l_2, v} |\Psi(\l_1,\l_2,v)|^2 < \infty
\end{equation}
$$

The action of the constraint on wave functions of $\l_1,\l_2$, and $v$ takes the following simplified form. 

$$
\begin{eqnarray}
\Theta \Psi(\l_1,\l_2,v) = \frac{\pi G}{2} \sqrt{v} \left[ -(v+2) \sqrt{v+4} \Psi_4^+(\l_1,\l_2,v) +(v+2) \sqrt{v} \right. \\
\left. \Psi_0^+ (\l_1,\l_2,v) + (v-2) \sqrt{v} \Psi_0^- (\l_1,\l_2,v) - (v-2) \sqrt{v-4} \Psi_4^-(\l_1,\l_2,v) \right]
\end{eqnarray}
$$

Which surprisingly acts as a difference operator in the volume v much like the FLRW models.  There is one term that shifts the volume forward by 4, two that leave the volume the same, and one that shifts the volume back by 4.  The complexity of the action of the constraint arises in how it changes the anistropies labelled by $\l_i$.  The action on the anisotropies is given by multiplicative shifts depending on the volume.

$$
\begin{align} \Psi^\pm_4(\l_1,\l_2,v)=& \:\Psi
\left(\f{v\pm4}{v\pm2}\cdot\l_1,\f{v\pm2}{v}\cdot\l_2,
v\pm4;\right)+\Psi\left(\f{v\pm4}{v\pm2}\cdot\l_1,\l_2,
v\pm4;\right)\nonumber\\& +\Psi\left(\f{v\pm2}{v}\cdot\l_1,
\f{v\pm4}{v\pm2}\cdot\l_2,v\pm4;\right)+\Psi
\left(\f{v\pm2}{v}\cdot\l_1, \l_2,v\pm4;\right)\nonumber
\\&+\Psi\left(\l_1,\f{v\pm2}{v}\cdot\l_2,
v\pm4;\right)+\Psi\left(\l_1,\f{v\pm4}{v\pm2}\cdot\l_2,v\pm4;\right),
\end{align}
$$

$$
\begin{align} \label{qham8} \Psi^\pm_0(\l_1,\l_2,v;)=
& \:\Psi\left(\f{v\pm2}{v}\cdot\l_1, \f{v}{v\pm2}\cdot\l_2,v;\right)
+\Psi\left(\f{v\pm2}{v}\cdot\l_1,\l_2,v; \right)\nonumber \\&
+\Psi\left(\f{v}{v\pm2}\cdot\l_1,\f{v\pm2}{v}\cdot\l_2,v;
\right)+\Psi\left(\f{v}{v\pm2}\cdot\l_1,\l_2,v;\right)\nonumber
\\& +
\Psi\left(\l_1,\f{v}{v\pm2}\cdot\l_2,v;\right)+\Psi\left(\l_1,\f{v\pm2}{v}
\cdot\l_2,v;\right)\, ,\end{align}
$$
...
$$
\begin{eqnarray}
U_1(a) \Psi(\l_1,\l_2,v) = \Psi(\l_1+a,\l_2,v) \\
...
$$
\begin{eqnarray}
x_i = \ln \left( \f{\sqrt{|p_i|}}{(4\pi |\gamma|\,
\sqrt\Delta\, \lp^3)^{1/3}} \right) \\
...
$$
\begin{equation}
\Psi_{FRW} = \sum_{\l_1,\l_2} \Psi(\l_1,\l_2,v)
\end{equation}
$$
...
$$
\begin{equation}
\Psi_{FRW} = \sum_{x_1,x_2}  \underset{\alpha \rightarrow \infty}{lim} \frac{1}{2\alpha_1} \int_{-\alpha_1}^{\alpha_1} \dd m_1  \underset{\alpha \rightarrow \infty}{lim} \frac{1}{2\alpha_2} \int_{-\alpha_2}^{\alpha_2} \dd m_2 e^{i m_1 x_1} e^{i m_2 x_2} \Psi(m_1,m_2,v)
\end{equation}
$$ 
...
$$
\begin{equation}
\Psi(v)_{FRW} =  \underset{\alpha \rightarrow \infty}{lim} \frac{1}{2\alpha_1} \int_{-\alpha_1}^{\alpha_1} \dd m_1  \underset{\alpha \rightarrow \infty}{lim} \frac{1}{2\alpha_2} \int_{-\alpha_2}^{\alpha_2} \dd m_2 \bar\delta(m_1) \bar\delta(m_2) \Psi(m_1,m_2,v) = \Psi(0,0,v)
\end{equation}
$$
