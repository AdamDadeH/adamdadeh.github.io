...
categories:
- Math
description: '"Coordinates" is underspecified in every standard definition. A better formulation: coordinates are a bijection that makes the relevant structures on a space efficiently computable. This reformulation connects directly to algorithmic complexity.'
---
...
Perhaps unsurprisingly the definitions are ambiguous — they describe coordinates for geometry without capturing what makes a coordinate system *good*. The missing ingredient is structure.
...
A distinct choice of coordinates for $\mathbb{N}$ comes from prime factorization, $n = 2^{m_2} 3^{m_3} 5^{m_5}\dots = \prod_{p \,\text{prime}} p^{m_p}$. This gives a unique bijection between the natural numbers $\mathbb{N}$ and the prime powers, $\prod_p \mathbb{N}$.
...
The broadest definition of coordinates — any bijection $f : X \rightarrow C$ — is correct but useless. What makes coordinates good is that they make the relevant structures on $X$ cheap to compute with in $C$.

This reframes the question: choosing coordinates for a mathematical structure and choosing a data representation for an algorithm are the same problem. The cost of evaluating a structure $\phi$ in coordinates $(f, C, \psi)$ is exactly the algorithmic complexity of computing $\phi$ using the primitives $\psi$. The eigenvalue decomposition, positional notation, and prime factorization are all answers to this question for different structures and different notions of cost.

The connection to machine learning runs the same way — learned representations are coordinate systems, and what gets optimized is precisely the cost of computing the task-relevant structure in those coordinates.
