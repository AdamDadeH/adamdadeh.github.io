...
- Quantum-Gravity
- Quantum-Cosmology
description: 'A fragment of unpublished research on Loop Quantum Cosmology: a simplification of the Bianchi I Hamiltonian constraint operator that reduces anisotropic dynamics to a tractable difference equation in volume.'
---
...
\begin{align}
p_1 &= \operatorname{sgn}(a_1) |a_2 a_3| L_2 L_3 \\
p_2 &= \operatorname{sgn}(a_2) |a_1 a_3| L_1 L_3 \\
p_3 &= \operatorname{sgn}(a_3) |a_1 a_2| L_1 L_2
\end{align}
...
We then see that taking $m_i=0$ is equivalent to enforcing isotropy, and indeed if $m_i=0$ the constraint reduces to that of the $k=0$ FLRW model.  How does this projection compare with the original?  The original projection introduced was,
...
\begin{align}
&-f_+^4(m_1,m_2,v) \Psi(m_1,m_2,v+4) + 12 v^2 \Psi(m_1,m_2,v) \\
...
Now we have an operator whose diagonal term is positive, so it is more natural to assume that the constraint is positive definite.  If it were we should see that the spectrum is continuous and runs from $(-m_1^2+m_2^2+(m_1-m_2)^2)$ to $\infty$.
