...
categories:
- Fun
description: 'Reverse-engineering Gran Turismo 2 save files: binary diffing with `cmp`, locating the CRC32, and patching bytes with `dd` and `xxd`.'
---

Playing Gran Turismo 2 with a young child created a conflict: frequent requests to buy and race new cars, zero interest in grinding for credits. Running on an emulator (OpenEmu + Mednafen on macOS) raised the possibility of editing save files directly. Thus began a detour into binary file forensics.
